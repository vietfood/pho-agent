...
export interface PlanAgentFeatureOptions {
  contextEnabledToolNames?: (entries: readonly unknown[]) => readonly string[] | undefined;
}

export function createPlanAgentFeature(options: PlanAgentFeatureOptions = {}): AgentFeature {
  return {
    id: PLAN_AGENT_FEATURE_ID,
    version: PLAN_AGENT_FEATURE_VERSION,
    extensionFactories: [createPlanAgentExtension(options)],
    expected: { extensions: 1 },
  };
}

export function createPlanAgentExtension(options: PlanAgentFeatureOptions = {}): InlineExtension {
  return {
          ...
          async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
            ...
            pi.setActiveTools(
              intersectPlanActiveTools({
                ...
                contextEnabledNames: options.contextEnabledToolNames?.(ctx.sessionManager.getEntries()),
                mode: next.mode,
